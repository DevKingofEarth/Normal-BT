...
class AndroidBluetoothController(
    ...
    override fun startBluetoothServer(): Flow<ConnectionResult> {
        return flow {
            if (!hasPermission(Manifest.permission.BLUETOOTH_CONNECT)){
                throw SecurityException("NOO BLUETOOTH_CONNECT permission")
            }
    ...
    override fun connectToDevice(device: BluetoothDeviceDomain): Flow<ConnectionResult> {
        return flow {
            if (!hasPermission(Manifest.permission.BLUETOOTH_CONNECT)){
                throw SecurityException("NOOO BLUETOOTH_CONNECT permission")
            }
...
//            }
            ...
            currentClientSocket?.let { socket->
                try {
                    ...
                    emit(ConnectionResult.Error("Connection Was Interrupt"))
                }
            }
        }.onCompletion {
            closeConnection()
    ...
    companion object{
        const val SERVICE_UUID = "1f2a5d4b-9d98-4c27-a9c8-75b6e4511e8a"
    }
}
